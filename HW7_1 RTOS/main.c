...
void thread_button_messages(void *p)
{
  ...
  {
    int a;
    // xQueueReceive is used to get an item from a queue; the first argument specifies 
    // the queue from which to receive; the second argument is the pointer to where data 
    // should be written from the queue; the third argument specifies how long to wait (in ticks) -- 
    // setting this to 0 says to wait indefinitely to get an element from the queue
    if  ( xQueueReceive(queue_button, (void *)(&a), 0) == pdTRUE)
    {
      // xSemaphoreTake is used to lock the mutex/semaphore; 
      // second argument is amount of time (in ticks) to wait to lock; 
      // if set to 10, for example, this function will wait for 10 ticks and 
      // if it could not lock successfully, it will return pdFALSE; if set to  portMAX_DELAY, 
      // it will wait indefinitely to lock; this function returns pdTRUE if it successfully locked
...
int main(void)
{
  ...
  // first argument to xQueueCreate is length of the queue (max number of items it can hold); 
  // second argument is size of each item (in bytes)
